		...
		if ($this->form_validation->run()) 
		{
			...
			if ($_FILES['account_picture_upload']['error'] != 4) 
			{
				...
				$this->load->library('upload', array(
					...
					'overwrite' => true,
					'upload_path' => FCPATH.'uploads/user/profile',
				...
				else 
				{
					...
					$this->image_lib->initialize(array(
						'image_library' => 'gd2',
						'source_image' => FCPATH.'uploads/user/profile/'.$picture['file_name'],
						'new_image' => FCPATH.'uploads/user/profile/pic_'.$picture['raw_name'].'.jpg',
					...
					{
						$data['account_details']->picture = 'uploads/user/profile/pic_'.$picture['raw_name'].'.jpg';
						$this->account_details_model->update($data['account']->id, array('picture' => $data['account_details']->picture));
					}
					
					// Delete original uploaded file
				
					unlink(FCPATH.'uploads/user/profile/'.$picture['file_name']);	
